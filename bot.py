...
while True:
  ...
  if prev_count != count:
    print(count)
  if 900000 < count < 1000000:
    if count % 100 == 0 or ((posted_count // 100) * 100 + 100) <= count:
      prompt = f"これはあなたの人格です。'{personality}'\nこの人格を演じて次の文章に対して80文字以内で返信してください。"
      text = f"ユーザー数が100万人になるまで100人ずつカウントアップしています。SNSのBlueskyのユーザーが{count}人になり100万人にもう少しであることをBlueskyのユーザーに向けて伝える投稿をしてください。人数は正確認書いてください。"
      answer = gpt.get_answer(prompt, text)
      post(session, answer)
    ...
    if posted_count < count:
      if count >= 1000000 == 0 and prev_count < 1000000:
        post(session, """🎀みなさま、わたくしBlueskyでございます🎀
おめでとうございますわ、皆様🎀わたくしのお城のお友達がついに100万人になりましたの。
これも一重に皆様のおかげでございます。
これからも皆様と一緒に楽しく、わいわいとお過ごしできますように。
わたくしBlueskyが全力でサポートいたしますわ。どうぞこれからもよろしくお願い申し上げます。お祝いの日にふさわしく、皆様の今日一日が喜びで満ち溢れますように🎀
#Bluesky #100万人達成 #感謝の気持ち #新たな冒険の始まり""")
      elif count >= 100000 == 0:
